powerenv-STDLIB 1 "2019" powerenv "User Manuals"
============================================

NAME
----

powerenv-stdlib - functions for the `.envrc`
...
Outputs a bash script called the *stdlib*. The following commands are included in that script and loaded in the context of an `.envrc`. In addition, it also loads the file in `~/.config/powerenv/powerenvrc` if it exists.
...
To find the value of the `integrity-hash`, call `powerenv fetchurl <url>` and
extract the hash from the outputted message.

See also `powerenv-fetchurl(1)` for more details.
...
### `powerenv_apply_dump <file>`

Loads the output of `powerenv dump` that was stored in a file.

### `powerenv_load [<command-generating-dump-output>]`

Applies the environment generated by running *argv* as a command. This is useful for adopting the environment of a child process - cause that process to run "powerenv dump" and then wrap the results with powerenv_load.

Example:

    powerenv_load opam-env exec -- powerenv dump
...
Creates and loads a virtualenv environment under `$PWD/.powerenv/python-$python_version`. This forces the installation of any egg into the project's sub-folder.

It's possible to specify the python executable if you want to use different versions of python (eg: `layout python python3`).

Note that previously virtualenv was located under `$PWD/.powerenv/virtualenv` and will be re-used by powerenv if it exists.
...
Sets the GEM_HOME environment variable to `$PWD/.powerenv/ruby/RUBY_VERSION`. This forces the installation of any gems into the project's sub-folder. If you're using bundler it will create wrapper programs that can be invoked directly instead of using the `bundle exec` prefix.
...
Adds each file to powerenv's watch-list. If the file changes powerenv will reload the environment on the next prompt.

Example (.envrc):

    watch_file Gemfile

### `powerenv_version <version_at_least>`

Checks that the powerenv version is at least old as `version_at_least`. This can
